...
pub async fn create_db_connection() -> Result<Surreal<Client>, Error> {
    ...
    let db_host = std::env::var("DATABASE_HOST_PAYMENTS").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_HOST_PAYMENTS not set")
    })?;
    let db_port = std::env::var("DATABASE_PORT_PAYMENTS").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_PORT_PAYMENTS not set")
    })?;
    ...
    let db_name: String = std::env::var("DATABASE_NAME_PAYMENTS").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_NAME_PAYMENTS not set")
    })?;
