...
pub async fn create_db_connection() -> Result<Surreal<Client>, Error> {
    ...
    let db_host = std::env::var("DATABASE_HOST_EMAIL").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_HOST_EMAIL not set")
    })?;
    let db_port = std::env::var("DATABASE_PORT_EMAIL").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_PORT_EMAIL not set")
    })?;
    ...
    let db_name: String = std::env::var("DATABASE_NAME_EMAIL").map_err(|e| {
        tracing::error!("Config Error: {}", e);
        Error::new(ErrorKind::Other, "DATABASE_NAME_EMAIL not set")
    })?;
