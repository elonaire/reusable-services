...
use crate::utils::{
    ...
    graphql_api::{perform_mutation_or_query_with_vars, perform_query_without_vars},
    models::{AuthStatus, SignInResponse, UserLogins, UserLoginsVar},
};
use hyper::HeaderMap;
...
pub async fn check_auth_from_acl(headers: HeaderMap) -> Result<AuthStatus, Error> {
    ...
    match headers.get("Authorization") {
        Some(auth_header) => {
            ...
            let endpoint =
                env::var("OAUTH_SERVICE").expect("Missing the OAUTH_SERVICE environment variable.");

            // let client = GQLClient::new_with_headers(endpoint, auth_headers);

            let auth_response = perform_query_without_vars::<CheckAuthResponse>(
                Some(auth_headers),
                endpoint.as_str(),
                gql_query,
            )
            ...
            match auth_response.get_data() {
                Some(auth_response) => Ok(auth_response.check_auth.to_owned()),
                None => Err(Error::new(
                    std::io::ErrorKind::Other,
                    "ACL server not responding! check_auth_from_acl",
                )),
            }
        }
...
pub async fn internal_sign_in() -> Result<String, Error> {
    ...
    let auth_response = perform_mutation_or_query_with_vars::<SignInResponse, UserLoginsVar>(
        None,
        endpoint.as_str(),
        gql_query,
        logins_var,
